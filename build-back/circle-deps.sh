...
if [ "${1-}" = "docker" ]; then
  ...
  time go install github.com/dmatrix/build-cache
...
# The tag for the dmatrix/builder image. If the image is changed
...
if ! docker images | grep -q "${tag}"; then
  ...
  if [[ ! -e "${builder}" ]]; then
    time docker pull "dmatrix/builder:${tag}"
    docker tag -f "dmatrix/builder:${tag}" "dmatrix/builder:latest"
    time docker save "dmatrix/builder:${tag}" > "${builder}"
  else
...
HOME="" go get -d -u github.com/dmatrix/build-cache
