...
// evictEntry will evict the entries from the minimum Index
// when the cache filled.
func (rec *raftEntryCache) evictEntry() {
	if len := rec.entryCache.Len(); len <= rec.size {
		return
	} else if len > rec.size {
		evictLen := len - rec.size
		for i := rec.firstIndex; i < rec.firstIndex+uint64(evictLen); i++ {
			rec.entryCache.Del(entryCacheKey(i))
		}
	}
	return
